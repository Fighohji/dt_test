...
class DtBiddingStrategy(BaseBiddingStrategy):
    ...
    def bidding(self, timeStepIndex, pValues, pValueSigmas, historyPValueInfo, historyBid,
        ...
        alpha = self.model.take_actions(test_state,
                                        pre_reward=sum(history_conversion[-1]) if len(history_conversion) != 0 else None, target_return=self.budget / self.cpa / 2000)
        

        bids = alpha * pValues
        # bids = 0.91 * alpha * pValues


        if budget_left - 1 / 48 > (48 - timeStepIndex - 1):
            bids = alpha * 1.2 * pValues
