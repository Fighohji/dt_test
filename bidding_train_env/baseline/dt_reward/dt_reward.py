...
class DecisionTransformer(nn.Module):
    ...
    def __init__(self, state_dim, act_dim, state_mean, state_std, action_tanh=False, K=10, max_ep_len=96, scale=2000,
        ...
        self.predict_cost = torch.nn.Linear(self.hidden_size * 2, 1)
        self.predict_reward = torch.nn.Linear(self.hidden_size * 2, 1)
    ...
    def forward(self, states, actions, rewards, timesteps, cost, attention_mask=None):
        ...
        cost_embeddings = self.embed_cost(cost)
        time_embeddings = self.embed_timestep(timesteps)

        state_embeddings = state_embeddings + time_embeddings
        action_embeddings = action_embeddings + time_embeddings
        rewards_embeddings = rewards_embeddings + time_embeddings
        cost_embeddings = cost_embeddings + time_embeddings

        # rewards ?
        stacked_inputs = torch.stack(
            (state_embeddings, action_embeddings, cost_embeddings, rewards_embeddings), dim=1
        ...
        cost_preds = self.predict_cost(x[:, 0:2])
        reward_preds = self.predict_reward(x[:, 0:2])
        return None, None, None, reward_preds, cost_preds

    def get_cost(self, states, actions, rewards, timesteps, costs, **kwargs):
        ...
        _, _, _, reward_preds, cost_preds = self.forward(
            states, actions, rewards, timesteps, costs, attention_mask=attention_mask, **kwargs)

        return reward_preds[0, -1], cost_preds[0, -1]

    def step(self, states, actions, rewards, dones, timesteps, costs, attention_mask):
        costs_target, rewards_target = torch.clone(costs), torch.clone(rewards)

        # 前向传播，得到花费的预测
        state_preds, action_preds, return_preds, rewards_preds, costs_preds = self.forward(
            states, actions, rewards, timesteps, costs, attention_mask=attention_mask,
        )

        cost_dim = costs_preds.shape[2]
        costs_preds = costs_preds.reshape(-1, cost_dim)[attention_mask.reshape(-1) > 0]
        costs_target = costs_target.reshape(-1, cost_dim)[attention_mask.reshape(-1) > 0]

        # 计算动作的损失
        costs_loss = torch.mean((costs_preds - costs_target) ** 2)
        rewards_loss = torch.mean((rewards_preds - rewards_target) ** 2)
        loss = costs_loss + rewards_loss
    ...
    def take_actions(self, state, action, target_return=None, pre_reward=None):
        self.eval()
        if self.eval_states is None:
            self.eval_states = torch.from_numpy(state).reshape(1, self.state_dim)
            self.eval_actions = torch.from_numpy(action).reshape(1, self.act_dim)
        else:
